...
def extract_file_paths(root_filepath, skip_folders, required_files):
    ...
    for dirpath, dirnames, filenames in os.walk(root_filepath):
        # parse files containing assurance_report
            ...
            for skip_string in skip_folders:
                ...
                if skip_string.lower() not in dirpath.lower():
                    for file_string in required_files:
                        # pull only paths from required files 
...
# filepaths - list of assurance report strings - pulls out data from report.
...
def extract_assurance_report_data(filepaths):
    ...
    for path in filepaths:
        ...
        for line in fhand:
            line = line.rstrip()
            split = line.split(': ')
            # add lines which have a character after ':' to file dict
            if len(split) == 2 and split[1] != '':
                # clean commas from ints
                if ',' in split[1]:
